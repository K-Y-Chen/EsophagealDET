...
def run(
    ...
    for batch_i, (im, targets, modal_targets, paths, shapes) in enumerate(pbar):
        ...
        # Inference
        with dt[1]:
            model_out, modal_out, iden = model(im) if compute_loss else (model(im, augment=augment), None)
            preds, train_out = model_out

        # Loss
        if compute_loss:            
            loss += compute_loss((model_out, modal_out, iden), targets, modal_targets.to(device))[1]  # box, obj, cls
