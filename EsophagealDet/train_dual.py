...
from models.yolo import Model, PromptSelfSupervisedDetectionModel
...
from utils.loss_tal_dual import ComputeLossPrompt as ComputeLoss
...
Model = PromptSelfSupervisedDetectionModel
...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    if pretrained:
        ...
        model = Model(cfg or ckpt['model'].yaml, ch=3, nc=nc, anchors=hyp.get('anchors'), nm=nm).to(device)  # create
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        LOGGER.info(('\n' + '%11s' * 9) % ('Epoch', 'GPU_mem', 'box_loss', 'cls_loss', 'dfl_loss', 'modal_loss', 'iden_loss', 'Instances', 'Size'))
        if RANK in {-1, 0}:
            pbar = tqdm(pbar, total=nb, bar_format=TQDM_BAR_FORMAT)  # progress bar
        optimizer.zero_grad()
        for i, (imgs, targets, modal_targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            with torch.cuda.amp.autocast(amp):
                ...
                # print(modal_targets)
                loss, loss_items = compute_loss(pred, targets.to(device), modal_targets.to(device))  # loss scaled by batch_size
                if RANK != -1:
            ...
            if f.exists():
                ...
                if f is best:
                    ...
                    results, _, _ = validate.run(
                        ...
                        callbacks=callbacks,
                        compute_loss=compute_loss,
                        modality=modality)  # val best model with plots
                    if is_coco:
    ...
    # Evolve hyperparameters (optional)
        ...
        for _ in range(opt.evolve):  # generations to evolve
            ...
            keys = ('metrics/precision', 'metrics/recall', 'metrics/mAP_0.5', 'metrics/mAP_0.5:0.95', 'val/box_loss',
                    'val/obj_loss', 'val/cls_loss', 'val/modal_loss', 'val/iden_loss')
