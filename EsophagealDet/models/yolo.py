...
class Resize(nn.Module):
    ...
    def forward(self, x):
        src, tgt = x
        res = F.interpolate(src, size=tgt.size()[2:], mode='bilinear', align_corners=True)
        return res


class FrontFeat(nn.Module):
    def __init__(self):
        super(FrontFeat, self).__init__()
    def forward(self, x):    
        return x


class BackFeat(nn.Module):
    def __init__(self):
        super(BackFeat, self).__init__()
    def forward(self, x):    
        return x
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    anchors, nc, gd, gw, act, nm = d['anchors'], d['nc'], d['depth_multiple'], d['width_multiple'], d.get('activation'), d.get('nm', 1)
    if act:
    ...
    # print(nc, nm)
    layers, save, c2, prompt = [], [], ch[-1], []  # layers, savelist, ch out
    try:
        component = d['prompt'] + d['backbone'] + d['head']
    except:
        component = d['backbone'] + d['head']
    
    for i, (f, n, m, args) in enumerate(component):  # from, number, module, args
...
class PromptSelfSupervisedDetectionModel(BaseModel):
    # TODO: implementation
    def __init__(self, cfg='yolo.yaml', ch=3, nc=None, anchors=None, nm=3):  # model, input channels, number of classes
        super().__init__()
        if isinstance(cfg, dict):
            self.yaml = cfg  # model dict
        else:  # is *.yaml
        ...
        # Define model
        ch = self.yaml['ch'] = self.yaml.get('ch', ch)  # input channels
        self.nm = self.yaml['nm'] = self.yaml.get('nm', nm)  # number of modality
    ...
    def forward(self, x, augment=False, profile=False, visualize=False):
        if augment:
            return self._forward_augment(x)  # augmented inference, None
        model_out, modal_out, iden = self._forward_once(x, profile, visualize)  # single-scale inference, train
        return model_out, modal_out, iden
    ...
    def _forward_once(self, x, profile=False, visualize=False):
        ...
        for m in self.model:
            ...
            if profile:
                self._profile_one_layer(m, x, dt)
            x = m(x)  # run
            if isinstance(m, FrontFeat):
                fn_feat.append(x)
            if isinstance(m, BackFeat):
                bk_feat.insert(0, x)
        ...
        return x, modal, (fn_feat, bk_feat)
