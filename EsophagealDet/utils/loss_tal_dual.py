...
class ComputeLossPrompt:
    ...
    def __init__(self, model, use_dfl=True):
        ...
        self.proj = torch.arange(m.reg_max).float().to(device)  # / 120.0
        self.use_dfl = use_dfl
        self.modal_BCE = nn.BCEWithLogitsLoss(reduction='mean')
        self.iden_loss = nn.MSELoss(reduction='none')
    ...
    def __call__(self, p, targets, m_target, img=None, epoch=0):
        fn_feat, bk_feat = p[2]
        modal_out = p[1]
        p = p[0]
        ...
        # modal loss
        # self.modal_BCE(modal_out, m_target)
        loss[3] = self.modal_BCE(modal_out, m_target)
        
        # iden loss
        for i in range(len(fn_feat)):
            loss[4] = loss[4] + self.iden_loss(F.sigmoid(bk_feat[i]), F.sigmoid(fn_feat[i])).mean()
        
        
        loss[0] *= 7.5  # box gain
        loss[1] *= 0.5  # cls gain
        loss[2] *= 1.5  # dfl gain
        loss[3] *= 0.5  # modal gain
        loss[4] *= 0.5  # iden gain

        return loss.sum() * batch_size, loss.detach()  # loss(box, cls, dfl, modal)