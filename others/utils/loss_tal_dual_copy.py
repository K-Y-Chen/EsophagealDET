...
class ComputeLoss:
    ...
    def __init__(self, model, use_dfl=True):
        ...
        self.device = device
        print(self.stride, self.nc, self.nl, self.no, m.reg_max)
        # exit()
    ...
    def __call__(self, p, targets, img=None, epoch=0):
        ...
        # print([xi.view(feats[0].shape[0], self.no, -1).shape for xi in feats])
        pred_distri, pred_scores = torch.cat([xi.view(feats[0].shape[0], self.no, -1) for xi in feats], 2).split(
            (self.reg_max * 4, self.nc), 1)
        pred_scores = pred_scores.permute(0, 2, 1).contiguous()
        pred_distri = pred_distri.permute(0, 2, 1).contiguous()
        # print(pred_scores.sigmoid())
        # print(pred_scores.shape)
        # # print(pred_distri.sigmoid())
        ...
        # print(self.BCEcls(pred_scores, target_scores.to(dtype)))
