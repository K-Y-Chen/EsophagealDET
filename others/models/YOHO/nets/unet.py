...
from .resnet101_frn import Resnet101, FilterResponseNormalization
...
class YOHOunet(nn.Module):
    def __init__(
        self, num_classes=4, pretrained=False, backbone="resnet34"
    ):
        super(YOHOunet, self).__init__()
        ...
        self.head = DetectorHead(nc=self.num_classes, ch=[16, 16], stride=[8.0, 8.0])
    ...
    def forward(self, inputs):
        ...
        # ret = self.head([up1, fuse])
        ret = self.head([self.pool(up1), self.pool(up1)])
...
if __name__ == '__main__':
    ...
    net = YOHOunet(num_classes=4, pretrained=False, backbone='resnet34').cuda()
    y = net(x)
    net = net.train()
    y = net(x)
    print(type(y))
